...
# We can split s into n/k substrings where each substring, ti, consists of a contiguous block of k
...
# that each character in ui occurs exactly once. 
# In other words, if the character at some index j in ti occurs at a previous index < j
# in ti, then do not include the character in string ui.
...
# Convert each ti to ui by removing any subsequent occurrences of 
...
def merge_the_tools(string, k):
    ...
    for letter in string:
        if count == k:
            ...
            substring = ""
            count = 0
        substring += letter if letter not in substring else ""
        count += 1
    print(substring)
